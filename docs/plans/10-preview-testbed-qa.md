...
- **Gating (no prod cost, no wire marker):** the panel mounts only on a preview origin — host matches `worldspring-pr-<N>.*` (the per-PR Worker name from `preview.yml:75-77`) — resolved client-side from `location.host`. Prod (`worldspring.*`) never mounts it, and there is **no** new `welcome` field to carry a "testbed" flag (the welcome body is unchanged, `protocol.ts:222-243`). A `?qa=0` escape hatch hides it for clean screenshots.
