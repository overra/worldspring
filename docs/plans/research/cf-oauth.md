...
- **Breaks (blocker for public visibility):** domain verification needs a DNS TXT record on the client URL's domain. `survival-game.adam-730.workers.dev` won't work — you can't create TXT records under `workers.dev`. **A custom domain is a hard prerequisite** for any public-visibility client. Private visibility (testing within Adam's own account) needs no domain and works today.
