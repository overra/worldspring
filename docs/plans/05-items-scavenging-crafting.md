...
This doc expands Worldspring's item economy from "walk into a building, grab what's on the
...
- **Crafting is a flat `RECIPES` array** in `packages/shared/src/items.ts`; the client sends
...
- **Items are pure data.** `ItemType` is a 14-member string union, `ItemDef` carries
  ...
  (`packages/shared/src/items.ts:1-54`). Combat never branches on weapon identity
  (`apps/game/src/server/systems/combat.ts:169-183` picks melee vs ranged from the equipped def).
- **The one behavior hack:** `raw_venison` is special-cased by type inside `useItem` —
  ...
  (`apps/game/src/server/systems/players.ts:344-358`, penalty constant `items.ts:100`). This is the
...
- **Loot is floor-spawn only.** `LootSpawn` points are generated inside buildings by the
  frozen `lRng` stream (`seed ^ 0x100c`, `packages/shared/src/world.ts:494-509`); tier mapping is
  ...
  Server stocks/respawns them from `LOOT_TABLES` (`apps/game/src/server/systems/loot.ts:49-104`).
...
- **Deer drop venison directly** — `killDeer` spawns a `raw_venison` loot entity where the
  deer fell; there is no deer corpse (`apps/game/src/server/systems/wildlife.ts:92-109`).
  `Corpse.kind` is `"player" | "zombie"` (`apps/game/src/server/systems/state.ts:120-131`,
  `packages/shared/src/protocol.ts:100-111`).
- **Temperature is modeled, insulation is not.** `tickSurvival` pulls `v.temp` down at
  ...
  modifier hook (`apps/game/src/server/systems/survival.ts:140-151`). Shelter = building footprint
...
- **Flashlight exists and renders.** `ITEM_DEFS.flashlight` (`items.ts:92`) drives a
  pooled spotlight on remote players holding it (`apps/game/src/client/render/entities/RemotePlayers.tsx:220-240`)
...
- **Inventory = flat 8 slots, hotbar IS the inventory.** `INVENTORY_SLOTS = 8`
  (`packages/shared/src/constants.ts:114`), `(ItemStack | null)[]` on `ServerPlayer`
...
- **Interaction is one id space + one prompt.** `pickup` resolves loot → corpse → airdrop
  ...
  prompt (`apps/game/src/client/net/NetSystem.tsx:111-144`, `clientWorld.promptLootId` in
...
- **Wire protocol** is a `t`-discriminated JSON union with no version field
  ...
  `packages/shared/src/protocol.ts`; this doc's wire changes are all additive and assume it.
- **Persistence:** world entities are `(kind, JSON)` rows in `world_state`, rewritten
  wholesale through `persistAll`'s single transaction (`apps/game/src/server/GameRoom.ts:598-606`,
...
- **Tab panel UI:** `InventoryPanel` in `apps/game/src/client/ui/HUD.tsx:204-272` renders slot rows
  with USE/DROP buttons from the zustand store (`apps/game/src/client/state/store.ts`); icons fall
...
**Craft flow** (server, new `craftItem` in `apps/game/src/server/systems/players.ts`):
...
`ServerConfig`/`PRESETS` design in `packages/shared/src/config.ts`; doc 03 only stubs the file)
...
**Per-kind tables** (server-only data, `packages/shared/src/items.ts` next to `LOOT_TABLES`,
...
container is searched-out**; absence = stocked. New `apps/game/src/server/systems/containers.ts`:
...
New constants in `packages/shared/src/constants.ts` (house rule — no system-local tunables):
...
1. **M1 — Catalog + use behaviors as data** *(Sonnet 4.8)*.
   Files: `packages/shared/src/items.ts`, `apps/game/src/server/systems/players.ts`,
   `packages/shared/src/constants.ts`, `apps/game/src/client/net/NetSystem.tsx` (UNKNOWN_DEF guard),
   `apps/game/src/client/render/entities/InputController.tsx` (F keybind, §6),
   `apps/game/src/client/ui/HUD.tsx` (`USABLE_KINDS` + `"tool"`), `packages/shared/src/protocol.ts`
...
3. **M3 — Deterministic containers in worldgen** *(Opus 4.8 — determinism-sensitive)*.
   Files: `packages/shared/src/world.ts` (WorldContainer + generation AFTER existing streams),
   `apps/game/src/client/render/world/` (instanced container meshes, BuildingTrim pattern).
...
4. **M4 — Container search loop** *(Opus 4.8 — protocol + persistence)*.
   Files: `protocol.ts` (+`search`, +`conts`), new `apps/game/src/server/systems/containers.ts`,
