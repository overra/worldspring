...
**RESOLVED (same day, [PR 91](https://github.com/overra/worldspring/pull/91)).** The `--churn=0` first cut came back *worse* (EMA 18.6 ms), killing the scenario-weight hypothesis, so per-phase tick telemetry was added (`/api/health tickPhases`, PR 91) and attributed the cost in one sample: **physics ~6 ms (~50%)** — `physics.step()` ran 4 Rapier substeps every tick against the at-cap static set (~4,000 colliders) with **every dynamic body asleep**, pure broadphase overhead since players/zombies are kinematic-sim, not Rapier bodies — and **broadcast ~2.4 ms (~25%)**, the known JSON-snapshot roadmap item. Fix: skip the substeps while no body is awake (with a `worldDirty` guard forcing one step after any collider/body add/remove, because `removeCollider`'s wake of resting bodies only materializes during a step — the replay harness's fell-under-crate probe caught the naive skip). Re-measured at cap: **tick EMA 3.67 ms** (physics phase 0.00–0.02 ms), back at the 07-08 baseline; `persistAll` gate PASS (0–2 ms saves); steady max 13 ms — the residual over the 10 ms gate is GC-spike noise (a 104 ms "save-tick peak" carried a 1–2 ms direct save cost). The 12.67 vs 18.6 ms spread between identical runs is local-machine variance; trust the phase EMAs, not single-run totals.
