...
import { INTERP_DELAY_MS, isAnimalSpecies } from "@worldspring/shared/constants";
...
function interpolateAnimals(a: BufferedSnap, b: BufferedSnap, t: number): void {
  ...
  for (const ab of b.animals) {
    ...
    const species = isAnimalSpecies(ab.species) ? ab.species : "deer";
    let view: AnimalView | undefined = views.get(ab.id);
    if (view === undefined) {
      view = { id: ab.id, species, x: ab.x, y: ab.y, z: ab.z, yaw: ab.yaw, state: ab.state };
