...
JSON.parse(localStorage.getItem('ws_settings')).state.quality  // auto-detected tier — force ?tier=high (or Esc menu) before A/B runs
__gl.getPixelRatio()                                            // expect 2 on the high tier
__gl.getContext().getContextAttributes()                        // expect antialias:false (SMAA in the composer is the AA)
```
